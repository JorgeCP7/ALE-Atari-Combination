...
from .utils import get_time, save_pkl, load_pkl, to_grayscale, crop_image
...
class Agent(BaseModel):
    ...
    for self.step in tqdm(range(start_step, self.max_step), ncols=70, initial=start_step):
      ...
      if self.step >= self.learn_start:
        if self.step % self.test_step == self.test_step - 1:
          ...
          #ESCRIBIR DATOS ESTADISTICOS EN FICHERO DE SALIDA PARA GRAFICAR
          
          fichero = open(self.checkpoint_dir + "estadisticas.txt", "a")
          if os.stat(self.checkpoint_dir + "estadisticas.txt").st_size == 0: #si el fichero esta vacio
            fichero.write("step;avg_r;avg_l;avg_q;avg_ep_r;max_ep_r;min_ep_r\n")
            
          salida = "%d;%.4f;%.6f;%3.6f;%.4f;%.4f;%.4f\n" \
              % (self.step+1, avg_reward, avg_loss, avg_q, avg_ep_reward, max_ep_reward, min_ep_reward)
          fichero.write(salida.replace(".", ","))
  ...
  def predict2(self, s_t):
      outputs = self.q.eval({self.s_t: [s_t]})
  ...
  def q_learning_mini_batch(self):
    ...
    #self.writer.add_summary(summary_str, self.step)              JORGE
  ...
  def build_dqn(self):
    ...
    with tf.variable_scope('summary'):
      ...
      #self.writer = tf.summary.FileWriter('./logs/%s' % self.model_dir, self.sess.graph)              JORGE
  ...
  def play(self, n_step=10000, n_episode=100, test_ep=None, render=False):
    if test_ep == None:
      test_ep = self.ep_end #0.1 para evitar bucles infinitos
...
######################################################
  ...
  def play2(self):
    ...
    if pathname == "":
      fichero = open("cnn-input.txt")
    else:
      fichero = open(pathname+"/cnn-input.txt")
      
    #HISTORY ALE
    ale_history = History(self.config)
    ...
    while j < 4:
      ...
      #Transformacion de screen a grayscale y 84x84
      #screen = screen.reshape((210,160,3))
      #screen = imresize(rgb2gray(screen)/255., (84,84))
      
      screen = screen.reshape((210,160,3))
      #imagen = Image.fromarray(screen, 'RGB')
      #imagen.show()
      #raw_input()
      screen = self.env.process_image(screen)
    
      #Incluir a la historia
      ale_history.add(screen)
    ...
    #Valores Q
    qs = self.predict2(ale_history.get())[0]
    
    if pathname == "":
      fichero = open("cnn-output.txt", "w")
    else:
      fichero = open(pathname+"/cnn-output.txt", "w")    

    j=0
    while j < len(qs):
      if j < len(qs) - 1:  
        fichero.write(str(qs[j])+"\n")
      else:
        fichero.write(str(qs[j]))
      j += 1
    ...
    img = Image.fromarray(array, 'RGB') #'L'
    #img.save('my.png')
    img.show()
    exit()
    ...
    screen = imresize(rgb2gray(screen)/255., (84,84)) #84 --> puede sacarse de 'config.screen_width' y etc.  
