...
*/
std::vector<float> SearchTree::normalize(std::vector<float> lista) {
...
/*
  Devuelve la lista de refuerzos por cada accion, obtenidos tras las simulacion
*/
vector<return_t> SearchTree::get_branch_rewards(void) {
	
	vector<return_t> branch_rewards;
	
	for(int i = 0; i < p_root->v_children.size(); i++) {
	  TreeNode* curr_child = p_root->v_children[i];
		branch_rewards.push_back(curr_child->branch_return);
	}
...
/*
  Imprime informacion sobre los refuerzos obtenidos de la simulacion 
  (esto antes estaba en get_best_action())
*/
void SearchTree::print_action_rewards() {
